...
public class TransactionSpecification implements Specification<Transaction> {
    ...
    public Predicate toPredicate(
            final @NonNull Root<Transaction> root,
            final @NonNull CriteriaQuery<?> query,
            final @NonNull CriteriaBuilder criteriaBuilder
    ) {

        Join<Transaction, TransactionCategory> expCatJoinRoot = root.join("transactionCategory", JoinType.LEFT);
        Join<Transaction, TransactionCategory> payMethJoinRoot = root.join("payMethod", JoinType.LEFT);
